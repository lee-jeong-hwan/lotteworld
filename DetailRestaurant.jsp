...
<%
	RestaurantDto dto = (RestaurantDto)request.getAttribute("dto");
%>
<!DOCTYPE html>
<html>
<head>
...
/*			$(".moremenu").css('display', 'none');
	...
	$(function() {
		...
		$(".plus").click(function() {
			...
			$(".realmap").css('left', '<%=dto.getZoomplus()%>');
			$(".realmap").css('top', '<%=dto.getZoomminus()%>');
		});
...
<body>
	...
	<div class="body">
		<div class="wrap">
			...
			<h2><%=dto.getName()%></h2>
			<%
			if(!(dto.getExplain().equals("null"))){
			%>
			<p class="subtitle"><%=dto.getExplain()%></p>
			<%
			} else {
			%>
				...
				<%
					if(dto.getArr()!=null){
						out.println("<div class='tag'>"
								+"<div>");
						for(int i = 0; i < dto.getArr().length; i++){
							out.println("<a class='tags' href='javascript:void(0))'>"+dto.getArr()[i]+"</a>");
						}
						out.println("</div>"
								+"</div>");
					} else {
      ...
      function go() {
    	  $(".playBtn").toggleClass("on");
				...
				<ul class="explain">
					...
					<li>
						<div class="realexplain">
							...
							<p class="text2">
								<strong><%=dto.getPhonenumber()%></strong>
							</p>
						</div> <!-- realexplain -->					
					</li>
					<%
			...
			<div class="main2">
				...
				<div class="mapArea">
					<p class="location"><%=dto.getDetaillocation()	%></p>
					<div class="map">
						<div class="realmap">
							<img src="images1/<%=dto.getMapurl()%>"/>
				...
				<%
					if(dto.getAroundgift() != null && dto.getAroundrestaurant() != null){
				%>
				<div class="subArea">
					<div class="aroundAttraction">
						<a class="two1 on" href="script:void(0)">
							<span>주변 레스토랑</span>
						</a>
						<a class="two2" href="script:void(0)">
							<span>주변 기프트샵</span>
						</a>
					</div>
					<div class="blocklist" style="height: 363.1px">
						<div class="blocklistwrap">
							<%
								for(int i = 0; i < dto.getAroundRI().length; i++){
							%>
							<a class="fir">
								<div class="blocktop">
									<span class="label"><%=dto.getAroundRL()[i]%></span>
									<img src="images1/<%=dto.getAroundRI()[i]%>"/>
								</div>
								<div class="blockbottom">
									<span class="name"><%=dto.getAroundRN()[i]%></span>
								</div>
							</a>
							<%
								}
								for(int i = 0; i < dto.getAroundGI().length; i++){
							%>
							<a class="sec" style="display: none;">
								<div class="blocktop">
									<span class="label"><%=dto.getAroundGL()[i]%></span>
									<img src="images1/<%=dto.getAroundGI()[i]%>"/>
								</div>
								<div class="blockbottom">
									<span class="name"><%=dto.getAroundGN()[i]%></span>
								</div>
							</a>
							<%
								}
				...
				<%
					...
					if(dto.getAroundattrac() != null){
				%>
				<div class="subArea">
					<div class="aroundAttraction">
						<a class="on" href="script:void(0)">
							<span>주변 어트랙션</span>
						</a>
					</div> <!-- aroundAttraction -->
					<div class="blocklist" style="height: 363.1px">
						<div class="blocklistwrap">
						 <%
						 	for(int i = 0; i < dto.getAroundAI().length; i++){
						 %>
							<a>
								<div class="blocktop">
									<span class="label"><%=dto.getAroundAL()[i]%></span>
									<img src="images1/<%=dto.getAroundAI()[i]%>"/>
								</div>
								<div class="blockbottom">
									<span class="name"><%=dto.getAroundAN()[i]%></span>
								</div>
							</a>
						 <%
						 	} 
				...
				if(dto.getAroundrestaurant() != null && dto.getAroundgift() == null){
				%>
					<div class="subArea">
					<div class="aroundAttraction">
						<a class="on" href="script:void(0)">
				...
				if(dto.getAroundattrac() == null && dto.getAroundrestaurant() == null && dto.getAroundgift() == null){
				%>
				...
				%>
				<div class="button">
					<a href="LotteController?command=Restaurant">목록</a>
				</div>
			</div> <!-- main2 -->
		</div> <!-- wrap -->
	</div> <!-- body -->
